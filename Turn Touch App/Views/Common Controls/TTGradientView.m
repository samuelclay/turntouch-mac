...
- (id)initWithFrame:(NSRect)frameRect Gradient:(NSGradient*)theGradient inBezier:(NSBezierPath *)bezierPath  {
    ...
    if (self) {
        if (theGradient) {
            [self setDrawBlock:^(void){
                [theGradient drawInBezierPath:bezierPath angle:-90];
            }];
        }
    }
...
//    NSLog(@"Drawing gradient view: %@", NSStringFromRect(dirtyRect));
    if (self.drawBlock) {
        self.drawBlock();
...
- (void)fadeToDrawBlock:(void (^)(void))newDrawBlock withDuration:(CGFloat)animDuration {
    ...
    TTGradientView *tempView = [[TTGradientView alloc] initWithFrame:self.frame];
